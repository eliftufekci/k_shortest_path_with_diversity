...
public class FindKSPD_yen extends BasePathFindingAlgorithm {
    ...
    private void generateSpurs(Path basePath, int dest, List<Path> acceptedPaths,
        ...
        // Calculate which accepted paths overlap with basePath at which prefix
        // in one pass — rather than separately for each spur node.
        int maxSpurIdx = baseRoute.size() - 1;

        // For each accepted path, how far does it match the baseRoute?
        ...
        for (int p = 0; p < acceptedPaths.size(); p++) {
            ...
            matchDepth[p] = depth; // this path overlaps with baseRoute up to index depth-1
        }
        ...
        for (int i = 0; i < maxSpurIdx; i++) {
            ...
            for (int p = 0; p < acceptedPaths.size(); p++) {
                // O(1) check — instead of O(i) comparison
                if (matchDepth[p] > i) {
                    List<Integer> route = acceptedPaths.get(p).getRoute();
                    if (route.size() > i + 1) {
