...
import core.Pair;
import core.Path;
import core.PrefixMap;
import core.SimilarityMetric;

public class FindKSPD_ extends BasePathFindingAlgorithm {

    private int  numberOfPathsExplored;
    private int  nextPathId;
    private long heapCounter = 0;

    public FindKSPD_(Graph graph, double threshold, SimilarityMetric metric) {
        super(graph, threshold, metric);
    }

    public FindKSPD_(Graph graph, double threshold) {
    ...
    private Path findNextPath(GraphState graphState,
        ...
        while (!globalPQ.isEmpty()) {
            ...
            // Clear lqMinKey entry (this entry is now "consumed")
            lqMinKey.remove(lq);

            if (lq.isEmpty()) continue;

            // Lazy-deletion: globalPQ entry doesn't match current top of LQ
            ...
            // If top changed after clearing inactives, re-add LQ 
            // and process with correct priority in the next iteration
