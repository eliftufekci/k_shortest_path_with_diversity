...
/**
 * Graph dosyalarını internetten indir ve hazırla
 *
 * Indirilen graflar:
...
public class DownloadGraphs {
    ...
    private static void downloadFile(String urlString, String destFile) throws IOException {
        ...
        // SSL sertifika doğrulama sorunlarını aşmak için trust-all yapılandırması
    ...
    /**
     * Dosyanın başındaki {@code linesToRemove} adet satırı siler.
    ...
    private static void removeHeaderLines(String filePath, int linesToRemove) throws IOException {
        System.out.println("Removing first " + linesToRemove + " header lines from: " + filePath);
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(original));
            ...
            for (int i = 0; i < linesToRemove; i++) {
                if (reader.readLine() == null) break; // dosya beklenenden kısa
            }
        ...
        System.out.println("Header lines removed from: " + filePath);
    }
    ...
    private static void downloadAndExtract(String url,
        ...
        if (!gzFile.exists()) {
            ...
            System.out.println("Archive already exists, skipping download: " + filenameGz);
        }
        ...
        if (!outFile.exists()) {
            ...
            System.out.println("File already exists, skipping extraction: " + filenameOut);
        }

        // GZIP dosyasını sil
        if (gzFile.exists()) {
            gzFile.delete();
            System.out.println("Removed archive: " + filenameGz);
        }

        System.out.println("Done: " + filenameOut);
    }

    /** Geriye dönük uyumluluk: header satırı silmeden indir+çıkar */
    private static void downloadAndExtract(String url,
                                           String filenameGz,
